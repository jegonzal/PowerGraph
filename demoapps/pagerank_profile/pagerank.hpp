...
std::istream& operator>>(std::istream& in, vertex_data& vdata) {
  return in >> vdata.value;
}
...
void load_truerank(const std::string& fname,
                   std::vector<graph_type::vertex_data_type>& truerank) {
  std::ifstream fin;
  fin.open(fname.c_str());
  graph_type::vertex_data_type tmp;
  if (!fin.good()) {
    logstream(LOG_WARNING) << "Fail loading true page rank file: " << fname << std::endl;
  }
  while(fin) {
    fin >> tmp;
    truerank.push_back(tmp);
  }
  fin.close();
}
