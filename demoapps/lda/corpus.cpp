...
corpus_type::corpus_type(const std::string& dictionary_fname, 
...
void corpus_type::load_dictionary(const std::string& fname) {
...
void corpus_type::load_counts(const std::string& fname) {
  std::ifstream fin(fname.c_str());    
  while(fin.good()) {
...
void corpus_type::shuffle_tokens() {
...
void split(const corpus_type& base, corpus_type& c1, corpus_type& c2, double prop_c1) {
  ...
  for(size_t i = 0; i < base.tokens.size(); ++i) {
    const token_type tok(base.tokens[i]);
