...
void initialize_clusters(){
   ...
   while(true){
     ...
     if (!ps.g->vertex_data(first).reported){
   ...
   cluster first_clust;
   assign(first_clust.location, ps.g->vertex_data(first).datapoint);
   first_clust.sum_sqr =sum_sqr(ps.g->vertex_data(first).datapoint); 
   ps.g->vertex_data(first).clusterhead = true;
   ps.g->vertex_data(first).current_cluster = ps.K-1; //start from last to first
   first_clust.num_assigned_points = 1;
   assign(first_clust.cur_sum_of_points, ps.g->vertex_data(first).datapoint);
   ...
   for (int i=0; i<ps.K-1; i++){
     ...
     add_tasks();
      ...
      for (int j=0; j< ps.M; j++){
        const vertex_data & data = ps.g->vertex_data(j);
      ...
      for (int j=0; j< ps.M; j++){
        if (loc >= distances[j] && loc < distances[j+1]){
          thenode = j; 
          thenode_dist = ps.g->vertex_data(j).min_distance; 
          assert(!ps.g->vertex_data(j).clusterhead);
          break;
        }
      }
      assert(thenode != -1);
      cluster cur_clust;
      assign(cur_clust.location,ps.g->vertex_data(thenode).datapoint);
      cur_clust.sum_sqr = sum_sqr(ps.g->vertex_data(thenode).datapoint);
      cur_clust.num_assigned_points = 1;
      assign(cur_clust.cur_sum_of_points, ps.g->vertex_data(thenode).datapoint);
      ps.g->vertex_data(thenode).clusterhead = true;
      ps.g->vertex_data(thenode).current_cluster = ps.K-2-i;
