 ...
 */
template<typename graph_type, typename update_functor>
void last_iter(){
  typedef graphlab::core<graph_type, update_functor> core_type;
  const size_t update_count = static_cast< core_type* >(ps.glcore)->last_update_count();

  std::cout << "Entering last iter with " << ps.iiter 
            << " total updates so far " << update_count << "." << std::endl;

  double res,res2;
  double rmse = (ps.algorithm != STOCHASTIC_GRADIENT_DESCENT && ps.algorithm != NMF) ? 
    agg_rmse_by_movie<graph_type>(res) : agg_rmse_by_user<graph_type>(res);
  //rmse=0;
  printf(ac.printhighprecision ? 
         ...
         ps.gt.current_time(),  runmodesname[ps.algorithm], 
         ps.iiter, calc_obj<graph_type>(res),  rmse, 
         calc_rmse_wrapper<graph_type>(ps.g<graph_type>(VALIDATION), true, res2));
  ps.iiter++;
