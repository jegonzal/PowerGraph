...
double calc_obj(double res){
  ...
  for (int i=0; i< ps.M; i++){
    const typename graph_type::vertex_data_type * data = &g->vertex_data(i);
    if (data->num_edges > 0){
...
template<typename graph_type>
void calc_stats2(const graph_type * gr, double&minU, double& maxU, 
                 int &moviewithoutedges, int& numedges, double &avgval, 
                 double& avgtime, double &minval, double &maxval, double&mintime, double&maxtime){
  for (int i=ps.M; i< ps.M+ps.N; i++){ 
...
template<>
void calc_stats2<graph_type_mult_edge>(const graph_type_mult_edge * gr, 
                                       double&minU, double& maxU, 
                                       int &moviewithoutedges, int& numedges, 
                                       double &avgval, double& avgtime, 
...
void calc_stats(testtype type){
  ...
  logstream(LOG_FATAL) 
    << "The following code makes no sense since edges[] is not defined!" << std::endl
    << "If this is reached please fix the code!" << std::endl;
  return;
  ...
  int numedges = 0;
  calc_stats2(gr, minU, maxU, moviewithoutedges, numedges, 
              avgval, avgtime, minval, maxval, mintime, maxtime);
...
template<>
void calc_rmse_edge<graph_type_mult_edge>(graph_type_mult_edge::edge_id_type iedgeid, 
                                          const graph_type_mult_edge *_g, 
                                          double & rmse, 
                                          const graph_type_mult_edge::vertex_data_type & data, 
                                          const graph_type_mult_edge::vertex_data_type & pdata, 
                                          int &e, int i){
...
double calc_rmse(const graph_type * _g, bool test, double & res){
  ...
  for (int i=ps.M; i< ps.M+ps.N; i++){
    ...
    foreach(typename graph_type::edge_id_type iedgeid, _g->in_edge_ids(i)) {
      const typename graph_type::vertex_data_type & pdata = 
        ps.g<graph_type>(TRAINING)->vertex_data(_g->source(iedgeid)); 
      calc_rmse_edge<graph_type>(iedgeid, _g, RMSE, data, pdata, e, i);       
    }
  }
  res = RMSE;
...
double agg_rmse_by_movie(double & res){
  ...
  double RMSE = 0;
  for (int i=ps.M; i< ps.M+ps.N; i++){ 
    const typename graph_type::vertex_data_type * data =
      &ps.g<graph_type>(TRAINING)->vertex_data(i);
    RMSE+= data->rmse;
  }
  res = RMSE;
