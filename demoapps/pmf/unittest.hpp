...
*/       assert(ps.U.rows() == 5 && ps.U.cols() == 5);
      ...
      case 91: //WEIGHTED_ALS: -Iter100... UV. objective=0.0207271, RMSE=0.0043/0.6344. Time to finish=0.00hr.
         ...
         assert(train_rmse < 0.0045);
         //assert(validation_rmse < 0.6840);
         assert(validation_rmse < 0.79 ); //DB: to verify on ubuntu 10.10, problem with 0.7512 (reported by Gregot Gorjanc)
...
void unit_testing(int unittest, command_line_options& clopts){
   ...
   if (unittest == 1){ //ALTERNATING LEAST SQUARES
      ac.datafile = "als"; ps.algorithm = ALS_MATRIX; ac.algorithm = ALS_MATRIX; ac.FLOAT = true; ac.als_lambda = 0.001; ac.debug = true; ac.matrixmarket=true;
    ...
    ac.datafile = "movielens"; ps.algorithm = SVD_PLUS_PLUS; ac.algorithm = SVD_PLUS_PLUS; clopts.set_ncpus(1); ac.debug = true; ac.maxval = 5; ac.minval = 1; ac.isfloat=true;
     ...
     ac.datafile = "panel7_mmwritten.dat"; ps.algorithm = NMF; ac.algorithm = NMF; ac.matrixmarket = true; ac.D = 5; ac.debug = true; ac.iter = 5;
      ...
      ac.datafile = "wals"; ac.algorithm = WEIGHTED_ALS; ps.algorithm = WEIGHTED_ALS; ac.FLOAT = true; ac.als_lambda = 0.001; ac.matrixmarket = true;
