...
void add_tasks(core & glcore){
  typedef typename core::graph_type graph_type;
  typedef typename core::update_functor_type update_functor_type;
  std::vector<typename graph_type::vertex_id_type> um;
  ...
  case ALS_TENSOR_MULT: {
    update_functor_type fun;
    glcore.schedule(um,fun);
    break;
  }
...
template<typename core>
void run_graphlab(core& glcore, typename core::graph_type* validation_graph){
  ...
  typedef typename core::graph_type graph_type;
  double res, train_rmse =  agg_rmse_by_movie<graph_type>(res), res2;
  double obj = calc_obj<graph_type>(res);
 ...
 * ==== SETUP AND START
 */
template<typename Graph, typename UpdateFunction>
void start(command_line_options& clopts) {
  typedef Graph graph_type;
  graphlab::core<Graph, UpdateFunction> glcore;
  if (ps.glcore == NULL) ps.glcore = &glcore;
  ...
  */
  graph_type& graph = glcore.graph();
  graph_type validation_graph;
  graph_type test_graph; 

  ps.verify_setup();
  glcore.set_options(clopts); 
  ...
  if (!ac.manualgraphsetup){
    if (!ac.loadgraph){
      load_pmf_graph(ac.datafile.c_str(), &graph, &graph, TRAINING);
      ps.set_graph(&graph, TRAINING);

      //read the vlidation data (optional)
      printf("loading data file %s\n", (ac.datafile+"e").c_str());
      load_pmf_graph<graph_type>((ac.datafile+"e").c_str(), &graph, &validation_graph, VALIDATION);
      ps.set_graph(&validation_graph, VALIDATION);

      //read the test data (optional)
      printf("loading data file %s\n", (ac.datafile+"t").c_str());
      load_pmf_graph<graph_type>((ac.datafile+"t").c_str(), &graph, &test_graph, TEST);
	...
	oarc << ps.M << ps.N << ps.K << ps.L << ps.Le << ps.Lt << ac.D;
        oarc << graph << validation_graph << test_graph;
        printf("Done!\n");
        fout.close();
	exit(0);
      }
      ...
      iarc >> graph >> validation_graph >> test_graph;
  ...
  if (ac.stats){
    calc_stats<graph_type>(TRAINING);
    calc_stats<graph_type>(VALIDATION);
    calc_stats<graph_type>(TEST);
    exit(0);
  }
  ...
  printf("%s for %s (%d, %d, %d):%d.  D=%d\n", 
         runmodesname[ac.algorithm], ps.tensor?"tensor":"matrix", 
         ps.M, ps.N, ps.K, ps.L, ac.D);
  
  init<graph_type>(&graph);

  if (ac.datafile == "netflix" || ac.datafile == "netflix-r"){
    ac.minval = 1; ac.maxval = 5;
  } else if ((ac.datafile == "kddcup" || ac.datafile == "kddcup2") 
             && ac.maxval == DEF_MAX_VAL){
  ...
  if (ps.algorithm != LANCZOS && ps.algorithm != SVD){
    double res2 = 0;
    double rmse =  calc_rmse_wrapper(&graph, false, res);
    printf(ac.printhighprecision ? 
           "complete. Objective=%g, TRAIN RMSE=%0.12f VALIDATION RMSE=%0.12f.\n" :
           "complete. Objective=%g, TRAIN RMSE=%0.4f VALIDATION RMSE=%0.4f.\n", 
           calc_obj<graph_type>(res), rmse, 
           calc_rmse(ps.g<graph_type>(VALIDATION), true, res2));
  } else {
  ...
  case SVD_PLUS_PLUS:
  case STOCHASTIC_GRADIENT_DESCENT:
    run_graphlab(glcore, &validation_graph);
    break;
     
  case LANCZOS:
    logstream(LOG_FATAL) << "Not Supported!" << std::endl;
    // lanczos<core>(glcore); 
    break;
...
int do_main(int argc, const char *argv[]){
  ...
  case SVD:
  case NMF:
    start<graph_type, user_movie_nodes_update_function<graph_type> >(clopts);
    break;
