...
% The parallel gibbs sampler is an optimized a c++ implementation of the
% discrete Gibbs samplers which uses multiple threads to accelerate the
% generation of a single sampling chain.  The parallel Gibbs sampler
% implements two algorithms described in the paper:
%
...
% parallelization of the classic Gibbs sampler.  The second algorithm is
% the Splash Gibbs sampler which incrementally builds thin junction trees.
%
% To use this function you must first construct a discrete factor graph
% which is simply a cell array of table factors:
%
%   factor{1} = table_factor( [1,2], log(rand(3,4)) );
%   factor{2} = table_factor( [2,3], log(rand(4,2)) );
%
% This creates a factorized model (with random tables) over the variables
% 1, 2, and 3. We can then run the CHROMATIC sampler by calling:
%
...
%        algorithm is simpler and faster.  For slowly mixing models use 
%        the 'SPLASH' algorithm.  In this case additional options will 
%        need to be set.
%    * nsamples: [Default: 10] The number of joint samples to collect.
%    * nskip: [Default: 10] The number of samples to skip between joint 
%        samples.  Because of the asynchronous nature of the algorithms 
%        more or than nskip samples may actually be skipped in practice.
%        In the 'SPLASH' algorithm nskip * nvariables single variable 
%        updates are computed before the next joint sample is constructed.
%    * ncpus: [Default: 2] The number of threads to use when running 
%        the inference algorithm. The number of cpus should be less than 
%        the number of variables and ideally not much larger than the
