...
public:
  ...
  void gather(icontext_type& context, const edge_type& edge) {
    ...
    token_asg_type& asgs = edata.asgs;
    // Assume: Document ----> word
    factor_type& doc_factor = context.vertex_data(edge.source()).factor;
    factor_type& word_factor = context.vertex_data(edge.target()).factor;
    ASSERT_EQ(doc_factor.size(), NTOPICS);
    ASSERT_EQ(word_factor.size(), NTOPICS);
    // Resample the topics
    foreach(topic_id_type& asg, asgs) {
      ...
      for(size_t t = 0; t < NTOPICS; ++t) {
        const double n_dt = std::max(doc_factor[t].value, count_type(0));
        const double n_wt = std::max(word_factor[t].value, count_type(0));
        const double n_t  = std::max(global_factor[t].value, count_type(0));
