...
struct vertex_data : public graphlab::IS_POD_TYPE {
  ...
  uint cur_links;
  bool active;
  vertex_data()  { kcore = 0; total_active = 0; total_links = 0; active = true; cur_links = 0;}
}; // End of vertex data

std::ostream& operator<<(std::ostream& out, const vertex_data& vdata) {
  return out << "kcore=" << vdata.kcore;
}
 ...
 */
class factorized_kcores : 
  public graphlab::iupdate_functor<graph_type, factorized_kcores>,
  public graphlab::IS_POD_TYPE {
private:
  size_t total_links, total_active;
public:
  factorized_kcores() { }
  double priority() const { return 1; }
  void operator+=(const factorized_kcores& other) { total_active += other.total_active; total_links += other.total_links; }
  ...
  void init_gather(iglobal_context_type& context) { total_links = 0; total_active = 0; }
  ...
  // Merge two factorized_kcores accumulators after running gather
  void merge(const factorized_kcores& other) { 
     total_links += other.total_links; 
     total_active += other.total_active;
   }

  // Update the center vertex
  void apply(icontext_type& context) {
     vertex_data & data = context.vertex_data();
     total_active = (data.active == true);
...
}; // end of factorized_kcores update functor
SERIALIZABLE_POD(factorized_kcores);


#if defined(FSCOPE)
typedef graphlab::distributed_fscope_engine<graph_type, factorized_kcores> engine_type;
#elif defined(SYNCHRONOUS_ENGINE)
typedef graphlab::distributed_synchronous_engine<graph_type, factorized_kcores> engine_type;
#else
typedef graphlab::distributed_engine<graph_type, factorized_kcores> engine_type;
#endif
...
int main(int argc, char** argv) {
  ...
  engine.schedule_all(factorized_kcores());
  dc.full_barrier();
      ...
      if (graph.l_get_vertex_record(i).owner == dc.procid()) {
        fout << graph.l_get_vertex_record(i).gvid << "\t" 
          ...
          graph.l_get_vertex_record(i).num_out_edges << "\t" 
             << graph.get_local_graph().vertex_data(i).kcore << "\t";
