...
public:
  ...
  void gather(icontext_type& context, const edge_type& edge) {
    ...
    // for(size_t i = 0; i < neighbor.latent.size(); ++i) {
    //   X(X.rows()-1, i) = neighbor.latent(i);
    // }
  ...
  void merge(const als_update& other) {
    ...
    X.conservativeResize(X.rows() + other.X.rows(), NLATENT);
    X.block(oldX_rows, 0, other.X.rows(), NLATENT) = other.X;
    y.conservativeResize(y.size() + other.y.size());
    y.segment(oldX_rows, other.y.size()) = other.y; 
  } // end of merge
  ...
  void apply(icontext_type& context) {
    ...
    ASSERT_GT(X.rows(), 0 );
    ASSERT_EQ(X.rows(), y.size());
    Eigen::MatrixXd XtX(NLATENT, NLATENT);
    XtX.triangularView<Eigen::Upper>() = X.transpose() * X;
    Eigen::VectorXd Xty(NLATENT);
    Xty = X.transpose() * y;
    // Clear the matrix
    X = Eigen::MatrixXd();
    y = Eigen::VectorXd();
