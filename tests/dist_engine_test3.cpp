...
void MultiplyFunction(gl_types::iscope& scope,
  ...
  if (vdata->DBLVAL < 1000.0) {
    foreach(edge_id_t edge, scope.out_edge_ids()) {
      const MyVertexData * rvdata = scope.neighbor_vertex_data(scope.target(edge)).as_ptr<MyVertexData>();
      vdata->DBLVAL += rvdata->DBLVAL;
  logger(LOG_INFO, 
         "    neighbor: %d, %lf", scope.target(edge), rvdata->DBLVAL);
