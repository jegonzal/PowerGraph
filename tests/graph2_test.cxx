...
public:
  ...
  struct update_functor_e : 
    public graphlab::iupdate_functor<graph_type_e, update_functor_e> {};
  ...
  */
   ...
   */
  void test_empty_edge_graph() {

    graph_type_e ge;

    std::cout 
      << "-----------Begin Empty Edge Grid Test--------------------" 
      << std::endl;
    ge.clear_reserve();
    size_t dim = 3;
    size_t num_vertices = 0;
    ...
    for (size_t i = 0;i < dim; ++i) {
      for (size_t j = 0;j < dim - 1; ++j) {
        // add the horizontal edges in one direction
        ge.add_edge(dim * i + j, dim * i + j + 1);
        // add the vertical edges in one direction
        ge.add_edge(dim * j + i, dim * (j + 1) + i);
        num_edge += 2;
      }
    }
    // the graph is now constructed
    // we need to call finalize. 
    ge.set_is_directed(false);
    ge.finalize();

    printf("Test num_vertices()...\n");
    ASSERT_EQ(ge.num_vertices(), num_vertices);
    printf("+ Pass test: num_vertices :)\n\n");

    printf("Test num_edges()...\n");
    ASSERT_EQ(ge.num_edges(), num_edge);
    printf("+ Pass test: num_edges :)\n\n");

    printf("Test iterate over in/out_edges and get edge data: \n");
    for (vertex_id_type i = 0; i < num_vertices; ++i) {
      const edge_list_type_e& out_edges = ge.out_edges(i);
      const edge_list_type_e& in_edges = ge.in_edges(i);
