...
public class Als {
  ...
  public static void main(String[] args) throws IOException, CoreException,
    ...
    SimpleWeightedGraph<AlsVertex, DefaultWeightedEdge> graph = new SimpleWeightedGraph<AlsVertex, DefaultWeightedEdge>(
   ...
   * 
   * In other words, populate the vector in each vertex with random
   ...
   *          number of latent factors
   */
  private static void randomLatentFactors(Graph<AlsVertex, ?> graph, int nlatent) {
  ...
  /**
   * Constraints <tt>value</tt> within {@link #LOWER_BOUND} and {@link #UPPER_BOUND}.
   * @param value
   * @return
   ...
   * @author Jiunn Haur Lim <jiunnhal@cmu.edu>
   */
  private static class AlsUpdater extends
      Updater<AlsVertex, DefaultWeightedEdge, AlsUpdater> {

    private double mError;
    private WeightedGraph<AlsVertex, DefaultWeightedEdge> mGraph;

    public AlsUpdater(WeightedGraph<AlsVertex, DefaultWeightedEdge> graph,
        double error) {
      mError = error;
      mGraph = graph;
    }
    ...
    public void update(Context context, AlsVertex vertex) {
      ...
      for (final DefaultWeightedEdge edge : edges) {
        // get neighbor
        DenseVector neighbor = Graphs.getOppositeVertex(mGraph, edge, vertex)
            .vector();
      ...
      DenseMatrix newLatent = DenseCholesky.factorize(XtX).solve(
          new DenseMatrix(Xty));
      vertex.setVector(new DenseVector(newLatent.getData()));

      // compute the residual change in the latent
      // factors-----------------------
      vertex.mResidual = 0;
      for (int i = 0; i < NLATENT; ++i)
      ...
      for (final DefaultWeightedEdge edge : edges) {
        ...
        double pred = truncate(vertex.vector().dot(neighbor.vector()));
        ...
        if (error > TOLERANCE && vertex.mResidual > TOLERANCE)
          context.schedule(neighbor, new AlsUpdater(mGraph, error
              * vertex.mResidual));
   ...
   */
  private static class AlsAggregator extends
      Aggregator<AlsVertex, AlsAggregator> {
    ...
    protected void exec(Context context, AlsVertex vertex) {
      ...
      mMaxRootMeanSquaredError = Math.max(mMaxRootMeanSquaredError,
          Math.sqrt(vertex.mSquaredError / numEdges));
    ...
    protected void add(AlsAggregator other) {
      ...
      mMaxRootMeanSquaredError = Math.max(mMaxRootMeanSquaredError,
          other.mMaxRootMeanSquaredError);
    }
