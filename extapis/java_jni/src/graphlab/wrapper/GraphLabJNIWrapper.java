...
/**
 * GraphLab JNI Wrapper. This will load the GraphLab C++ library for multi-core
 * and interface with it. Ensure that libgraphlab.{so,dylib} is in java.library.path.
 * 
 * In general, applications will maintain their own graph data in the Java
 * layer. The engine manipulates a proxy graph and forwards update calls to the
 * application. Note the GraphLab2 does not support multiple update functions.
 * 
 * @author akyrola
...
public class GraphLabJNIWrapper {
	...
	/** Singleton */
	private static GraphLabJNIWrapper graphlab = null;

	// load the JNI library
	static { System.loadLibrary("graphlabjni"); }
	
	/**
	 * Obtains the singleton instance. The JNI library maintains a
	 * single graphlab core, and so there can at most be one instance
	 * of graphlab running per application at any one time.
	 * @return JNI wrapper for GraphLab
	 */
	public static GraphLabJNIWrapper getInstance(){
		if (null == graphlab) graphlab = new GraphLabJNIWrapper ();
		return graphlab;
	}
	...
	/**
	 * Initializes the GraphLab core. The JNI library maintains a
	 * single graphlab core only. Multiple calls to this method will 
	 * manipulate the same core and possibly override previous settings.
	 * This also means that there can at most be one instance of graphlab
	 * (per application) at any one time.
	 * TODO allow user to configure log levels and other engine options
	 */
	private native void initGraphLab();
	 ...
	 */
	private GraphLabJNIWrapper(UpdateFunction... updateFunctions) {
		initGraphLab();
	}
...
//
//	/**
//	 * Set maximum number of iterations (only valid for some schedulers, such as
...
//
//	/**
//	 * Set the number of parallel threads to use. Default = all. You can also
//	 * use command line argument <tt>-Dgraphlab.ncpus=X</tt>.
//	 * 
//	 * @param ncpus
//	 */
...
//
//	/**
//	 * Set the name of the scheduler to use. Examples: multiqueue_fifo, colored,
//	 * sweep, priority, round_robin, sampling, multiqueue_priority
//	 * 
//	 * @param s
//	 */
...
//
//	/**
//	 * If using a colored scheduler, use this to automatically compute a greedy
//	 * graph coloring for the graph. You can also do coloring by hand with
//	 * graph.setColor().
//	 */
//	native public void computeGraphColoring();
//
//	/**
//	 * Pass the graph. You must have created the whole graph at this point, and
//	 * cannot add vertices or edges after this.
//	 * 
//	 * @param graph
//	 */
...
//
//	/**
//	 * Convenience function for adding the first update function to the schedule
//	 * for all vertices.
//	 */
...
//
//	/**
//	 * Internal method. Called by the GraphLab wrapper in order to execute an
//	 * update function on a vertex.
//	 * 
//	 * @param vertexId
//	 * @param functionId
...
//			JniScheduler scheduler = new JniScheduler(Math.max(1,
//					scope.getNumOutboundEdges()),
