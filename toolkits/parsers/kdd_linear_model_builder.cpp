...
int main(int argc,  char *argv[]) {
  ...
  clopts.attach_option("validation_data", &validation_data, validation_data, "validation data");
  clopts.add_positional("validation_data");
  ...
  load_matrixmarket_graph(validation_data, validation_info,
										      validation, MATRIX_MARKET_4);
  load_matrixmarket_graph(user_data_file, user_data_info, user_data);
  load_matrixmarket_graph(item_data_file, item_data_info, item_data);

  vec training_rating = zeros(training.num_edges());
  vec validation_rating = zeros(training.num_edges());

  graph_type  out_graph;
  out_graph.resize(training.num_edges()+MAX_FEATURE);
  graph_type out_graph_validation;
  out_graph_validation.resize(training.num_edges()+MAX_FEATURE);
  int training_instance = 0;
  int added_training = 0; 
  int added_validation = 0;
  ...
  for (int i=0; i< nodes; i++){
     ...
     for (uint j=0; j < out_edges.size(); j++){
       training_instance++;
       uint user = out_edges[i].source();
       uint item = out_edges[i].target() - nodes; 
       edge_data2 & edge = training.edge_data(out_edges[i]);
       ...
       for (uint k=0; k < user_features.size(); k++){
         ...
         if (edge.time > split_training_time){
           out_graph_validation.add_edge(training_instance, pos, newedge); 
           added_validation++;
           validation_rating[training_instance] = rating;
         }
         else {
           out_graph.add_edge(training_instance, pos, newedge);
           added_training++;
       ...
       for (uint k=0; k < item_features.size(); k++){
         ...
         ASSERT_GE(pos , 0);
         ASSERT_LE(pos, MAX_FEATURE);
         edge_data edge2 = user_data.edge_data(item_features[k]);
         assert(edge2.weight >= 0);
         edge_data newedge(edge2.weight);
         if (edge.time > split_training_time){
           out_graph_validation.add_edge(training_instance, pos, newedge);
  ...
  bipartite_graph_descriptor out_info, out_info_validation;
  out_info_validation.rows = out_info.rows = training.num_edges();
  out_info_validation.cols = out_info.cols = MAX_FEATURE;
  out_info.nonzeros = out_graph.num_edges();
  out_info_validation.nonzeros = out_graph_validation.num_edges();
  out_graph.finalize();
  out_graph_validation.finalize();

  save_matrixmarket_graph(outfile +".train", out_info, out_graph);
  save_matrixmarket_graph(outfile +".validation", out_info_validation, out_graph_validation);
