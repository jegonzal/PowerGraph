...
signed int current_max = 0;
unsigned int max_vid = -1;
...
// typedef std::pair<int, int> adj_pair;
// std::set<adj_pair, Cmp> adjacent;
  ...
  std::set<IntPair, Comparator> adj(c);
...
public:
  ...
  bool is_adj_and_of_degree(const graph_type::vertex_type& vertex) {
    return vertex.data().degree == current_max;
  }
...
int main(int argc, char** argv) {
  ...
  dc.cout() << "Finding starting node (max degree vertex)..." << std::endl;
  ...
  dc.cout() << "Starting at node " << v.vid << " with degree " << v.degree << std::endl;
  engine.signal(v.vid);
  engine.start();
