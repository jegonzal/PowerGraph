...
namespace graphlab {
  ...
  public:
    ...
    // Remote calls
    static void remote_set_value(distributed_control& _dc, 
                                 size_t source, void *ptr, size_t len,
                                 std::string key, double value);
        
    static void remote_start_bandwidth_test(distributed_control& _dc, 
                                            size_t source, void *ptr, 
                                            size_t len);
 
    static void receive_bandwidth_test_payload(distributed_control& _dc, 
                                               size_t source, void *ptr, 
                                               size_t len);
...
#define BANDWIDTH_TEST_SIZE (30*1024*256)
    ...
    void execute_bandwith_test() {
      ...
      // Fill with random data to avoid any compression (I think there
      // isnt, but just in case)
      ...
      for(int j=0; j<dc->numprocs(); j++) {
        ...
        if (i != dc->procid()) {
          dc->remote_callxs(i, distributed_metrics::
                            remote_start_bandwidth_test, NULL, 0);
          dc->remote_callxs(i, distributed_metrics::
                            receive_bandwidth_test_payload, 
                            payload, BANDWIDTH_TEST_SIZE);
        } else {
    ...
    void start_bandwidth_test(size_t from_proc) {
      ...
      std::cout << "Start bandwidth test from " 
                << from_proc << " to " << dc->procid() 
                << std::endl;
    }
    ...
    void end_bandwith_test(size_t from_proc, size_t len) {
      ...
      received_payloads++;
      std::cout << "Finished " 
                << received_payloads << ". bandwidth test from " 
                << from_proc << " to " << dc->procid() << ": " 
    ...
    double get_value(int procid, std::string key) {
      ...
      if (iter == stats[procid].end()) {
        ASSERT_MSG("A key %s was not found for procid %d!", 
                   key.c_str(), procid);
