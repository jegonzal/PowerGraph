 ...
 * George
 *
 * $Id: kwayvolfm.c,v 1.3 2003/07/31 06:04:52 karypis Exp $
 *
 */
...
**************************************************************************/
void Random_KWayVolRefine(CtrlType *ctrl, GraphType *graph, idxtype nparts, float *tpwgts, 
                          float ubfactor, idxtype npasses, idxtype ffactor)
{
  idxtype i, ii, iii, j, jj, k, kk, l, u, pass, nvtxs, nmoves, tvwgt, myndegrees, xgain; 
  idxtype from, me, to, oldcut, oldvol, vwgt;
  ...
  IFSET(ctrl->dbglvl, DBG_REFINE,
     mprintf("VolPart: [%5D %5D]-[%5D %5D], Balance: %3.2f, Nv-Nb[%5D %5D]. Cut: %5D, Vol: %5D\n",
...
* This function performs k-way refinement
**************************************************************************/
void Random_KWayVolRefineMConn(CtrlType *ctrl, GraphType *graph, idxtype nparts, float *tpwgts, 
            float ubfactor, idxtype npasses, idxtype ffactor)
{
  idxtype i, ii, iii, j, jj, k, kk, l, u, pass, nvtxs, nmoves, tvwgt, myndegrees, xgain; 
  idxtype from, me, to, oldcut, oldvol, vwgt, nadd, maxndoms;
  idxtype *xadj, *adjncy, *adjwgt;
  idxtype *where, *pwgts, *perm, *bndptr, *bndind, *minwgt, *maxwgt, *itpwgts, *updind, *marker, *phtable;
...
* This function performs k-way refinement
**************************************************************************/
void Greedy_KWayVolBalanceMConn(CtrlType *ctrl, GraphType *graph, idxtype nparts, float *tpwgts, 
                                float ubfactor, idxtype npasses)
{
  idxtype i, ii, iii, j, jj, k, kk, l, u, pass, nvtxs, nmoves, tvwgt, myndegrees, xgain; 
  idxtype from, me, to, vwgt, gain, maxndoms, nadd;
  idxtype *xadj, *adjncy, *adjwgt;
  idxtype *where, *pwgts, *perm, *moved, *bndptr, *bndind, *minwgt, *maxwgt, *itpwgts, *updind, *marker, *phtable;
  idxtype *pmat, *pmatptr, *ndoms;
  VEDegreeType *myedegrees;
...
**************************************************************************/
idxtype ComputeVolume(GraphType *graph, idxtype *where)
{
  idxtype i, j, k, me, nvtxs, nparts, totalv;
  ...
  nparts = where[idxargmax(nvtxs, where)]+1;
...
* This function computes the initial id/ed 
**************************************************************************/
void CheckVolKWayPartitionParams(CtrlType *ctrl, GraphType *graph, idxtype nparts)
{
...
**************************************************************************/
void ComputeVolSubDomainGraph(GraphType *graph, idxtype nparts, idxtype *pmat, idxtype *ndoms)
{
...
void EliminateVolSubDomainEdges(CtrlType *ctrl, GraphType *graph, idxtype nparts, float *tpwgts)
{
  ...
  for (;;) {
    ...
    /* mprintf("Adjacent Subdomain Stats: Total: %3D, Max: %3D, Avg: %3D\n", total, max, avg); */
