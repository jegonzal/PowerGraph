...
#ifndef GRAPHLAB_DISTRIBUTED_CONTEXT_HPP
#define GRAPHLAB_DISTRIBUTED_CONTEXT_HPP
...
namespace graphlab {
  ...
  private:    
    ...
    vertex_id_type local_vid;
    vertex_id_type vid;

    /** The consistency model that this distributed_context ensures */
    consistency_model _consistency;

    std::pair<vertex_id_type, vertex_id_type> alt_cache;  // a 1 entry global->local cache
  public:
  ...
  public:
     ...
     * function.
     */
    distributed_context(engine_type* engine_ptr = NULL,
            graph_type* graph_ptr = NULL,
            vertex_id_type global_vid = -1,
            consistency_model consistency = EDGE_CONSISTENCY) :
      engine_ptr(engine_ptr), graph_ptr(graph_ptr), 
      local_vid(-1), vid(global_vid), _consistency(consistency),
      alt_cache(vertex_id_type(-1),vertex_id_type(-1)) {
        if (vid != vertex_id_type(-1) && graph_ptr != NULL) {
          local_vid = graph_ptr->local_vid(vid);
        }
      }
    
    
    void init(const vertex_id_type global_vertex, consistency_model consistency) {
      vid = global_vertex; _consistency = consistency;
      local_vid = graph_ptr->local_vid(global_vertex);
      ASSERT_TRUE(graph_ptr != NULL);
      ASSERT_TRUE(engine_ptr != NULL);
    } // end of init_vertex

    void init_from_local(const vertex_id_type local_vertex, consistency_model consistency) {
      local_vid = local_vertex; _consistency = consistency;
      vid = graph_ptr->global_vid(local_vid);
      ASSERT_TRUE(graph_ptr != NULL);
      ASSERT_TRUE(engine_ptr != NULL);
    } // end of init_vertex

    void cache_vid_conversion(vertex_id_type local_vertex) {
      alt_cache.first = graph_ptr->global_vid(local_vertex);
      alt_cache.second = local_vertex;
    }
    ...
    //! Get the consistency model under which this distributed_context was acquired
    ...
    void schedule(const vertex_id_type& vertex, 
      ...
      if (vertex == alt_cache.first) engine_ptr->schedule_local(alt_cache.second, update_fun);
      else if (vertex == vid) engine_ptr->schedule_local(local_vid, update_fun);
      else engine_ptr->schedule(local_vid, update_fun);
    }
