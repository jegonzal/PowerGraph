...
#ifndef GRAPHLAB_GRAPH_LOCK_HPP
#define GRAPHLAB_GRAPH_LOCK_HPP
#include <list>
...
#define COMPILER_WRITE_BARRIER
  ...
  */
  ...
  class graph_lock {
  public:
    typedef typename GraphType::global2localvid_type global2localvid_type;
    ...
    */
    void scope_request(vertex_id_type globalvid,
      ...
      typename global2localvid_type::const_iterator 
  ...
  private:
    ...
    */
    ...
    void continue_partial_lock(typename lazy_deque<partiallock_cont_params>::value_type* ptr) {
      ...
      local_edge_list_type inedges = dgraph.localstore.in_edge_ids(curv);
      local_edge_list_type outedges = dgraph.localstore.out_edge_ids(curv);
    
      vertex_id_type inv  = 
        (inedges.size() > params.inidx) ? 
        dgraph.localstore.source(inedges[params.inidx]) : vertex_id_type(-1);
      vertex_id_type outv  = 
        (outedges.size() > params.outidx) ? 
        dgraph.localstore.target(outedges[params.outidx]) : vertex_id_type(-1);
      ...
      while (params.inidx < inedges.size() || 
        ...
        if (!params.curlocked && curv < inv  && curv < outv) {
          ...
          inv  = (inedges.size() > params.inidx) ? dgraph.localstore.source(inedges[params.inidx]) : (vertex_id_type)(-1);
        } 
        else if (outv < inv) {
          ++params.outidx;
          COMPILER_WRITE_BARRIER;
          ...
          outv  = (outedges.size() > params.outidx) ? dgraph.localstore.target(outedges[params.outidx]) : (vertex_id_type)(-1);
        } 
        else if (inv == outv){
    ...
    */
    ...
    void partial_unlock(vertex_id_type globalvid, size_t scopetypeint) {
      ...
      local_edge_list_type inedges = dgraph.localstore.in_edge_ids(localvid);
      local_edge_list_type outedges = dgraph.localstore.out_edge_ids(localvid);
      ...
      vertex_id_type inv  =  (inedges.size() > inidx) ? 
        dgraph.localstore.source(inedges[inidx]) : vertex_id_type(-1);
      vertex_id_type outv  = (outedges.size() > outidx) ? 
        dgraph.localstore.target(outedges[outidx]) : vertex_id_type(-1);
      ...
      while (inidx < inedges.size() || outidx < outedges.size()) {
        if (!curunlocked && curv < inv  && curv < outv) {
          ...
          inv = (inedges.size() > inidx) ? dgraph.localstore.source(inedges[inidx]) : (vertex_id_type)(-1);
        } 
        else if (outv < inv) {
          ++outidx;
          if (dgraph.localvid_is_ghost(outv) == false) {
          ...
          outv = (outedges.size() > outidx) ? dgraph.localstore.target(outedges[outidx]) : (vertex_id_type)(-1);
        } 
        else if (inv == outv){
          ++inidx; ++outidx;
          if (dgraph.localvid_is_ghost(outv) == false) {
