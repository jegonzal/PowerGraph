...
namespace graphlab {
  ...
  partition_atoms(const atom_index_file& atomindex, size_t nparts) {
    ...
    logstream(LOG_INFO) << "Balance factor (max / avg): " 
                        << (float(partition_weights.top().second) /
    ...
    if (missingparts.size() > 0) {
      logstream(LOG_WARNING) 
        << "Metis generated only " << nparts - missingparts.size() 
        << " when " 
        << nparts << " was requested. Attempting to repartition." 
        << std::endl;
    }
    ...
    for (size_t i = 0;i < missingparts.size(); ++i) {
      ...
      std::vector<std::vector<size_t> > bisect = 
        bisect_atoms_metis(atomindex, ret[repartidx]);
    ...
    if (missingparts.size() > 0) {
      logstream(LOG_WARNING) 
        << "Repartition successful. New balance factor (max / avg): " 
        << (float(partition_weights.top().second) /
            (float(totalweight) / nparts)) << std::endl;
