/**  
 * Copyright (c) 2009 Carnegie Mellon University. 
 *     All rights reserved.
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing,
 *  software distributed under the License is distributed on an "AS
 *  IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 *  express or implied.  See the License for the specific language
 *  governing permissions and limitations under the License.
 *
 * For more about this software visit:
 *
 *      http://www.graphlab.ml.cmu.edu
 *
 */


// Modified from boost 1.37 boost::any
// Extended to handle graphlab graphlab/serialization/deserialization functions
// See http://www.boost.org/libs/any for Documentation.

#ifndef GRAPHLAB_ANY_VECTOR_INCLUDED
#define GRAPHLAB_ANY_VECTOR_INCLUDED

// what:  variant type boost::any
// who:   contributed by Kevlin Henney,
//        with features contributed and bugs found by
//        Ed Brey, Mark Rodgers, Peter Dimov, and James Curran
// when:  July 2001
// where: tested with BCC 5.5, MSVC 6.0, and g++ 2.95

#include <algorithm>
#include <typeinfo>
#include <map>
#include <iostream>
#include <vector>
#include <stdint.h>

#include <boost/type_traits/remove_reference.hpp>
#include <boost/type_traits/is_reference.hpp>
#include <boost/throw_exception.hpp>
#include <boost/static_assert.hpp>
#include <boost/utility.hpp>
#include <boost/exception/detail/is_output_streamable.hpp>
#include <boost/functional/hash.hpp>


#include <graphlab/logger/assertions.hpp>
#include <graphlab/serialization/serialization_includes.hpp>

#include <graphlab/util/generics/any.hpp>

namespace graphlab {
  /**
   A generic "variant" object obtained from Boost::Any_Vector and modified to
   be serializable. A variable of type "any_vector" can store any_vector datatype
   (even dynamically changeable at runtime), but the caveat is that
   you must know the exact stored type to be able to extract the data
   safely.
   
   To serialize/deserialize the any_vector, regular serialization procedures
   apply.  However, since a statically initialized type registration
   system is used to identify the type of the deserialized object, so
   the user must pay attention to a couple of minor issues.
   
   On serialization: 
   
   \li \b a) If an any_vector contains a serializable type, the any_vector can be
             serialized.
   \li \b b) If an any_vector contains an unserializable type, the
             serialization will fail at runtime.
   
   On deserialization:
   
   \li \b c) An empty any_vector can be constructed with no type information
             and it can be deserialized from an archive.
   \li \b d) However, the deserialization will fail at runtime if the
             true type of the any_vector is never accessed / instantiated
             any_vectorwhere in the code.
             
   Condition \b d) is particular unusual so I will illustrate with an
   example.
   
   Given a simple user struct:
   \code
   struct UserStruct {
     int i;
     void save (graphlab::oarchive& oarc) const {
       oarc << i;
     }
     void load (graphlab::iarchive& iarc) {
       iarc << i;
     }
   }
  \endcode
  
   If an any_vector object contains the struct, it will be serializable.
   
   \code
   UserStruct us;
   us.i = 10;
   any_vector a = us;
   // output file
   std::ofstream fout("test.bin");
   graphlab::oarchive oarc(fout);
   oarc << a;    // write the any_vector
   \endcode
   
   To deserialize, I will open an input archive and stream into an any_vector.
   
   \code
   // open input 
   std::ifstream fin("test.bin");
   graphlab::iarchive iarc(fin);
   // create an any_vector and read it
   any_vector a;
   iarc >> a; 
   \endcode
   
   Now, unusually, the above code will fail, while the following code
   will succeed
   
   \code
   // open input 
   std::ifstream fin("test.bin");
   graphlab::iarchive iarc(fin);
   // create an any_vector and read it
   any_vector a;
   iarc >> a; 
   std::cout << a.as<UserStruct>().i;
   \endcode
   
   The <tt> a.as<UserStruct>() </tt> forces the instantiation of static functions
   which allow the any_vector deserialization to identify the UserStruct type.
  */
  class any_vector {
  private:
    /**
     * iholder is the base abstract type used to store the contents
     */
    class iholder {
    public: // structors
      virtual ~iholder() { }
      virtual const std::type_info& type() const = 0;
      virtual iholder * clone() const = 0;
      virtual uint64_t deserializer_id() const = 0;
      virtual void deep_op_equal(const iholder* c) = 0;
      static iholder* load(iarchive_soft_fail &arc);
      virtual void save(oarchive_soft_fail& arc) const = 0;
      virtual std::ostream& print(std::ostream& out) const = 0;
      virtual bool empty() const = 0;
      virtual size_t size() const = 0;
      virtual any get(const size_t index) const = 0;
      virtual void set(const size_t index, const any& other) = 0;
      virtual void resize(const size_t new_size) = 0;
    };
    iholder* contents;

  public: // structors
    /// default constructor. Creates an empty any_vector
    any_vector() : contents(NULL) { }

    /// Creates an any_vector which stores the value
    template<typename ValueType>
    any_vector(const size_t size, const ValueType& value = ValueType())
      : contents(new holder<ValueType>(size, value)) { }

    template<typename ValueType>
    any_vector(const std::vector<ValueType>& vec)
      : contents(new holder<ValueType>(vec)) { }


    /// Construct an any_vector from another any_vector
    any_vector(const any_vector & other) : 
      contents(other.unset() ? NULL : other.contents->clone()) { }

    /// Destroy the contentss of this any_vector
    ~any_vector() { delete contents; }

    /// Returns true if the object does not contain any_vector stored data
    bool unset() const { return contents == NULL; }

    /// Returns true if the vector is empty
    bool empty() const { return unset() || contents->empty(); }

    /// returns the size of the internal vector
    size_t size() const { return unset()? 0 : contents->size(); }

    /// returns the size of the internal vector
    void resize(const size_t size) { 
      if(unset()) {
        logstream(LOG_FATAL) 
          << "Cannot resize an unset any vector!" << std::endl;
      } else { contents->resize(size); }
    }


    /// returns the size of the internal vector
    template<typename ValueType>
    void resize(const size_t size, const ValueType& value) { 
      if (!unset() && type() == typeid(ValueType)) {
        static_cast< holder<ValueType> *>(contents)->contents.resize(size, value);
      } else {
        if(!unset()) { delete contents; contents = NULL; }
        contents = new holder<ValueType>(size, value);
      } 
    }


    /// Extracts a reference to the contents of the any_vector as a type of
    /// ValueType
    any get(const size_t index) const {      
      DASSERT_FALSE(unset());
      ASSERT_LT(index, contents->size());
      return contents->get(index);
    }

    /// Extracts a reference to the contents of the any_vector as a type of
    /// ValueType
    void set(const size_t index, const any& other) const {      
      DASSERT_FALSE(unset());
      ASSERT_LT(index, contents->size());
      contents->set(index, other);
    }



    /// Extracts a reference to the contents of the any_vector as a type of
    /// ValueType
    template<typename ValueType>
    ValueType& as(const size_t index) {
      DASSERT_TRUE(type() == typeid(ValueType));
      DASSERT_FALSE(unset());
      return static_cast<holder<ValueType> *>(contents)->contents[index];
    }

    /// Extracts a constant reference to the contents of the any_vector as a
    /// type of ValueType
    template<typename ValueType>
    inline const ValueType& as(const size_t index) const{
      DASSERT_TRUE(type() == typeid(ValueType));
      DASSERT_FALSE(unset());
      return static_cast< holder<ValueType> *>(contents)->contents[index];
    }


    /// Extracts a reference to the contents of the any_vector as a type of
    /// ValueType
    template<typename ValueType>
    std::vector<ValueType>& as() {
      DASSERT_TRUE(type() == typeid(ValueType));
      DASSERT_FALSE(unset());
      return static_cast<holder<ValueType> *>(contents)->contents;
    }

    /// Extracts a constant reference to the contents of the any_vector as a
    /// type of ValueType
    template<typename ValueType>
    inline const std::vector<ValueType>& as() const{
      DASSERT_TRUE(type() == typeid(ValueType));
      DASSERT_FALSE(unset());
      return static_cast< holder<ValueType> *>(contents)->contents;
    }

    /// Exchanges the contents of two any_vector's
    any_vector& swap(any_vector & rhs) {
      std::swap(contents, rhs.contents);
      return *this;
    }

    /**
     * Update the contents of this any_vector.  If a new type is used than
     * the type of this any_vector will change.
     */
    template<typename ValueType>
    any_vector& operator=(const std::vector<ValueType>& rhs) {
      if (contents != NULL && contents->type() == typeid(ValueType)) {
        as<ValueType>() = rhs;
      } else { any_vector(rhs).swap(*this); }
      return *this;
    }    
    /**
     * Update the contents of this any_vector to match the type of the other
     * any_vector.
     */
    any_vector& operator=(const any_vector & rhs) {
      if (rhs.unset()) {
        if (contents) { delete contents; contents = NULL; }
      } else {
        if (!unset() && contents->type() == rhs.contents->type()) {
          contents->deep_op_equal(rhs.contents);
        } else { any_vector(rhs).swap(*this); }
      }
      return *this;
    }

    std::ostream& print(std::ostream& out) const {     
      return unset()? (out << "EMPTY") : contents->print(out);        
    }

    /// Returns the type information of the stored data.
    const std::type_info& type() const {
      return unset() ? typeid(void) : contents->type();
    }
    
    /// Return the name of the internal type as a string.
    const std::string type_name() const {
      return unset() ? "NULL" : std::string(contents->type().name());
    }

    /// loads the any_vector from a file.
    void load(iarchive& arc) {
      iarchive_soft_fail isoftarc(arc);
      if(contents != NULL) { delete contents; contents = NULL; }
      bool isempty(true);
      isoftarc >> isempty;
      if (isempty == false) contents = iholder::load(isoftarc);      
    }
    
    /// Saves the any_vector to a file. Caveats apply. See the main any_vector docs.
    void save(oarchive& arc) const {
      oarchive_soft_fail osoftarc(arc);
      bool isempty = unset();
      osoftarc << isempty;
      if (isempty == false) contents->save(osoftarc);
    }


  public:
    /**
     * This section contain the global registry used to determine the
     * deserialization code for a particular type.  Essentially the
     * registry is a global map in which all subtypes of iholder
     * register a deserialization function with their type. 
     */

    typedef iholder* (*deserialize_function_type)(iarchive_soft_fail& arc);
    typedef std::map<uint64_t, deserialize_function_type> registry_map_type;
    /**
     * The get registry routine is a static method that gets a
     * reference to the global registry.  It is very important that
     * this be a static method and not a static member to ensure that
     * the global registry is defined before each holders try to
     * register.  This is accomplished by having get_registry
     * statically declare the global registry
     */
    static registry_map_type& get_global_registry();
    
  public:

    template <typename ValueType> static
    typename boost::disable_if_c<boost::is_output_streamable<ValueType>::value, 
                                 void>::type 
    print_type_or_contents(std::ostream& out, 
                           const std::vector<ValueType>& vec) { 
      out << "Not_Printable[" << typeid(ValueType).name() << ']'; 
    }

    template <typename ValueType> static
    typename boost::enable_if_c<boost::is_output_streamable<ValueType>::value, 
                                void>::type 
    print_type_or_contents(std::ostream& out, 
                           const std::vector<ValueType>& vec) { 
      out << "{";
      for(size_t i = 0; i < vec.size(); ++i ) 
        out << vec[i] << (i+1 < vec.size()? ", " : "}");      
    }


  public:

    /**
     * holder is an instantiation of iholder
     */
    template<typename ValueType>
    class holder : public iholder {
    public: 
      typedef ValueType value_type;   
      /// The actual contents of the holder
      std::vector<ValueType> contents;
      /// Construct a holder from a value
      holder(const std::vector<ValueType>& contents) : 
        contents(contents) { }
      /// Construct a holder from a value
      holder(const size_t size, const ValueType& value) : 
        contents(size, value) { }

      /// Construct a holder from an archive
      holder(iarchive_soft_fail& arc) { arc >> contents; }
      /// Get the type info of the holder
      const std::type_info& type() const { return typeid(ValueType); }
      /// Clone a holder
      iholder* clone() const { return new holder(contents); }
      /// Deep assignment
      void deep_op_equal(const iholder* other) {
        contents = static_cast< const holder<ValueType>* >(other)->contents;
      }
      /**
       * Get the deserializer id from the static registry associated
       * with this type of holder
       */
      uint64_t deserializer_id() const { return registry.localid; }
      void save(oarchive_soft_fail &arc) const { 
        arc << registry.localid << contents; 
      }    
      /**
       * Print the contents or the type if the contents does not
       * support printing
       */
      std::ostream& print(std::ostream& out) const {
        any_vector::print_type_or_contents(out, contents);
        return out;
      }
      bool empty() const { return contents.empty(); }
      size_t size() const { return contents.size(); }
      any get(const size_t index) const { return any(contents[index]); }
      void set(const size_t index, const any& other) {
        if(other.type() != type()) {
          logstream(LOG_FATAL) 
            << "Cannot assign " << other.type().name() << " to a vector of "
            << type().name() << "!" << std::endl;
        } else { contents[index] = other.as<ValueType>(); }
      }
      void resize(const size_t new_size) { contents.resize(new_size); }

      /** The actual deserialization function for this holder type */
      static iholder* deserialize(iarchive_soft_fail &arc) {
        return new holder(arc);
      }
      /**
       * The following struct defines the static member used to
       * automatically register the deserialization function for this
       * holder type and cache a shared id used to quickly identify
       * the deserialization function.
       *
       * Note that the registry actually uses the NAME of the type so
       * renaming a type will result in an incompatible
       * deserialization.
       */
      struct registry_type {
        uint64_t localid; 
        registry_type() { 
          boost::hash<std::string> hash_function;
          // compute localid
          localid = hash_function(typeid(ValueType).name());
          any_vector::get_global_registry()[localid] = holder::deserialize;
        }        
      }; // end of registry type
      /**
       * The registry is a static member that will get constructed
       * before main and used to register the any_vector type
       */
      static registry_type registry;        
    private: 
      holder& operator=(const holder& other) { }
    }; // end of class holder

  }; // end of class any_vector
  

  /**
   * This static membery computes the holder (type specific)
   * deserialization id and also registers it with the global
   * registry.
   */
  template<typename ValueType>
  typename any_vector::holder<ValueType>::registry_type 
  any_vector::holder<ValueType>::registry;

} // namespace graphlab

std::ostream& operator<<(std::ostream& out, 
                         const graphlab::any_vector& any_vector);


// Copyright Kevlin Henney, 2000, 2001, 2002. All rights reserved.
//
// Distributed under the Boost Software License, Version 1.0. (See
// accompanying file LICENSE_1_0.txt or copy at
// http://www.boost.org/LICENSE_1_0.txt)

#endif

