...
namespace graphlab {
  ...
  */
  ...
  public:
    ...
    virtual void init(asm_state<enumerator_type>& state, 
                      asm_callback_type& callback) = 0;
    virtual asm_action_args advance(asm_state<enumerator_type>& state, 
                                    asm_callback_type& callback) = 0;
  };
  ...
  */
  ...
  public:
    asm_collection(ExecutorImplementation& impl, 
                   size_t hashpoolsize = 100000):
      impl(impl), statemachine(statemachine) {
    ...
    bool set_state_of_machine(asm_machine_id_type machineid, 
                              const asm_state_type &state) {
      bool ret = false;
      size_t lockid = machineid % lockset.size();
      lockset[lockid].lock();
      // look for the machine id
