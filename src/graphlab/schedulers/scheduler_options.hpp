...
class scheduler_options {
 public:
   ...
   */
  inline void add_option_str(const std::string &opt, const std::string &val) {
   ...
   */
  inline bool get_string_option(const std::string &opt, std::string &val) const{
    std::map<std::string, scheduler_option_values>::const_iterator i = options.find(opt);
  ...
  inline bool get_int_option(const std::string &opt, IntType &val) const{
    std::map<std::string, scheduler_option_values>::const_iterator i = options.find(opt);
    if (i == options.end()) return false;
    val = i->second.intval;
    return true;
  ...
  inline bool get_float_option(const std::string &opt, FloatType &val) const{
    std::map<std::string, scheduler_option_values>::const_iterator i = options.find(opt);
    if (i == options.end()) return false;
    val = i->second.dblval;
    return true;
