...
#define __SCHEDULER_LIST__ (("colored", colored_scheduler, \
                            ...
                            (("fifo", fifo_scheduler, \
                              "Standard FIFO task queue, poor parallelism, but task evaluation sequence is highly predictable. Useful for debugging and testing."))  \
                            (("priority", priority_scheduler, \
                              "Standard Priority queue, poor parallelism, but task evaluation sequence is highly predictable. Useful for debugging"))  \
                            (("multiqueue_fifo", multiqueue_fifo_scheduler, \
                              "One or more FIFO task queues is assigned to each processor, where the queues are stochastically load balanced. Like the fifo scheduler, but less predictable, and much faster."))  \
                            (("multiqueue_priority", multiqueue_priority_scheduler, \
                              "One or more Priority task queues is assigned to each processor, where the queues are stochastically load balanced. Like the priority scheduler, but less predictable, and much faster."))  \
                            (("splash", splash_scheduler, \
                              "Similar to the priority queue scheduler, but allows for only one update function. Updates are evaluted in a \"splash\" ordering"))  \
                            ...
                            (("clustered_priority", clustered_priority_scheduler, \
                              "Like the priority scheduler, but groups vertices into clusters where the entire cluster has a single priority"))  \
                            (("sampling", sampling_scheduler, \
                              "A scheduler which samples vertices to update based on a multinomial probability which can be updated dynamically."))
