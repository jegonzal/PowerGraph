...
std::vector<std::string> get_scheduler_names() {
  ...
  #define __APPEND_TO_RET__(r_unused, data_unused, i,  elem) ret.push_back(BOOST_PP_TUPLE_ELEM(3,0,elem));
...
std::string add_line_breaks(const std::string &s, size_t numcols) {
  ...
  while(pos < s.length() - 1) {
    ...
    size_t newpos = pos;
    // search backward for a space if we are not at the end of the string
    if (pos < s.length()) {
      newpos = s.rfind(" ", pos);
    }
    // if we get back to the old position, or we fail to find a space, force the break
