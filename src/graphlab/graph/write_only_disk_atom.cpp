...
void write_only_disk_atom::play_back_disk(disk_atom* atom) {
  f.close();
...
void write_only_disk_atom::play_back_memory(memory_atom* atom) {
  f.close();
  std::ifstream fin(filename.c_str(), std::ios::binary);
  // flush the commands
  iarchive iarc(fin);
