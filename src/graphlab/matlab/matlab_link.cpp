#include "matlab_link.h"

void emx_get_edge_data(uint32_T handle, uint32_T eid, gl_emx_edgetype *edge) {
}
void emx_get_vertex_data(uint32_T handle, uint32_T vid, gl_emx_vertextype *vertex) {
}

void emx_set_edge_data(uint32_T handle, uint32_T eid, gl_emx_edgetype *edge) {
}

void emx_set_vertex_data(uint32_T handle, uint32_T vid, gl_emx_vertextype *vertex) {
}
