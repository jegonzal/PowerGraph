...
namespace mrf {
  ...
  /** 
   * Get the next given the current root the number of roots and the
   * number of vertices.  Vertices are partitioned into sets to ensure
   * that the next root remains within the set.
   */
  ...
  // Start a new tree
  void start_tree_update(gl::iscope& scope, 
    ...
    bool root_candidate = !vdata.in_tree;
    ...
    if(root_candidate) {
      foreach(edge_id_t eid, out_edges) {
        ...
        if(neighbor_vdata.in_tree) { root_candidate = false; break; }
      }
    } // end of if
