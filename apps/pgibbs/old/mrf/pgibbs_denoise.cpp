...
void run_colored_samples(denoise_problem& problem,
  ...
  std::vector<binary_factor> edge_factors(1);
  edge_factors[0] = problem.edge_factor;
  
  // Setup the engine
  gl::thread_shared_data sdm;
  gl::iengine* engine =
    make_colored_engine(graph, sdm, edge_factors, ncpus);
...
int main(int argc, char** argv) { 
  ...
  for(size_t i = 0; i < problem.graph.num_vertices(); ++i) {
    ...
    vdata.asg = rand() % vdata.potential.arity();
  }
