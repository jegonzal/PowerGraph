...
void reduce_RMSE(size_t index, const gl_dtypes::ishared_data& shared_data,
     ...
     new_data = double(curval + scope.vertex_data().rmse);
}
...
static void apply_QQR(size_t index,  const gl_dtypes::ishared_data& sdm,
   ...
   if (debug && ((int)index == 0 || (int)index == K-1)){
        printf("entering apply QQR node  %d \n", (int)index);   
   } 
  ...
  rmse = sqrt(rmse/L);
  std::cout << "Apply QQR: Training RMSE is : " << rmse << std::endl;
  // write the final result into the shared data table
  ret.rmse = rmse;
}
       ...
       foreach(edge_id_t iedgeid, _g->in_edge_ids(i)) {
         ...
         for (int j=0; j< (int)edges.medges.size(); j++){       
          ...
          //assert(add<25 && add>= 0);
           ...
           // DB: need to fix this to aggregate results over time
           //if (BPTF && iiter > BURN_IN){
           //  add = pow((edge.avgprd / (iiter - BURN_IN)) - edge.weight, 2);
           //  assert(edge.avgprd != 0);
           //}
  ...
  ASSERT_LE(scope.vertex(), M+N);
    ...
    if ((int)scope.vertex() < M){
	 ...
	 for (int j=0; j< (int)medges.medges.size(); j++){
              ...
              //                         T_update_function),1);
              double sum;     
              double trmse = gl::rmse(vdata.pvec, const_cast<double*>(pdata.pvec), tensor? vec2vec(&tvec.vals[(int)edge.time]) :NULL, D, edge.weight, sum);
              assert(sum != 0);
              if (BPTF && iiter > BURN_IN){
                 edge.avgprd += sum;       
...
int start(int argc, char ** argv, distributed_control & dc) {
  ...
  distributed_graph<vertex_data,multiple_edges>::partition_graph_tofile(g, o.ncpus, graphlab::partition_method::PARTITION_METIS, infile);
  exit(0);
  }*/
  ...
  graphlab::distributed_shared_data<gl_dtypes::graph> sdm(dc);
   ...
   graphlab::distributed_round_robin_scheduler<gl_dtypes::graph> > > *tengine = new graphlab::distributed_engine<gl_dtypes::graph,
   graphlab::distributed_scheduler_wrapper<gl_dtypes::graph, 
   graphlab::distributed_round_robin_scheduler<gl_dtypes::graph> > >(dc, dg, o.ncpus);
   tengine->set_caching(true);
   tengine->set_constant_edges(true);
   tengine->set_vertex_scope_pushed_updates(true);
   tengine->set_default_scope(scope_range::VERTEX_CONSISTENCY);
 ...
 if(tengine == NULL) {
  ...
  dc.barrier();
  tengine->set_shared_data_manager(&sdm);  
  dc.barrier();
   ...
   sdm.set_tree_sync(RMSE, reduce_RMSE, apply_func, merge_RMSE, double(2.5),  50000000); 
  ...
  if (tensor){ 
    ...
    sdm.set_tree_sync(TIME_OFFSET, sync_QQR, apply_QQR,merge_mult, mult, 400000000, M, M+N);
    sdm.atomic_set(TIME_OFFSET, multret);
  }

  if (BPTF){
     sdm.set_tree_sync(A_U_OFFSET, sync_MMT, apply_MMT_U,merge_QQR, QQR(), 400000000, 0, M);
     sdm.set_tree_sync(A_V_OFFSET, sync_MMT, apply_MMT_V,merge_QQR, QQR(), 40000000, M, M+N);
  }
...
//   tengine->get_scheduler().add_tasks(um, pmf_update_function, 1);
   ...
   tengine->get_scheduler().set_option(scheduler_options::MAX_ITERATIONS, (void*)20);
