...
#include <graphlab/distributed/distributed_round_robin_scheduler2.hpp>
...
const int NUM_ITERATIONS_TO_RUN = 5;
...
static void merge_mult(size_t index, const gl_dtypes::ishared_data& shared_data,
     ...
     assert(newd.vals.size() == curd.vals.size() && newd.vals.size() == K);  
 
     for (int i=0; i< newd.vals.size(); i++){ 
...
void init_self_pot(graphlab::distributed_shared_data<gl_dtypes::graph> &sdm){
  //assert(BPTF);
  ...
  ASSERT_LE(scope.vertex(), M+N);
   ...
   for (int j=0; j< (int)medges.medges.size(); j++){
    ...
    if (sdm->get_constant(IS_ROUND_ROBIN_CONSTANT).as<bool>() == false) {
      if (vdata.rounds < sdm->get_constant(MAX_ITERATIONS_CONSTANT).as<size_t>()) {
...
void barrier_fn(){ 
  ...
  ((graphlab::distributed_engine<gl_dtypes::graph,
    graphlab::distributed_scheduler_wrapper<gl_dtypes::graph, 
    graphlab::distributed_round_robin_scheduler2<gl_dtypes::graph> > > *)(tengine))->sync_barrier();
}
...
int start(int argc, char ** argv, distributed_control & dc) {
  ...
  graphlab::distributed_shared_data<gl_dtypes::graph> sdm(dc);
  ...
  if (o.scheduler == "round_robin") {
    graphlab::distributed_engine<gl_dtypes::graph,
    graphlab::distributed_scheduler_wrapper<gl_dtypes::graph, 
    graphlab::distributed_round_robin_scheduler2<gl_dtypes::graph> > > *d_engine = new  
                graphlab::distributed_engine<gl_dtypes::graph,
                graphlab::distributed_scheduler_wrapper<gl_dtypes::graph, 
                graphlab::distributed_round_robin_scheduler2<gl_dtypes::graph> > >(dc, dg, o.ncpus);
    d_engine->set_caching(true);
    //d_engine->set_vertex_scope_pushed_updates(true);
    //d_engine->set_constant_edges(true);
    d_engine->set_default_scope(scope_range::EDGE_CONSISTENCY);
    d_engine->set_max_backlog(1000);
    d_engine->set_shared_data_manager(&sdm);  
    tengine = d_engine;
  }
  else if (o.scheduler == "multiqueue_fifo") {
    graphlab::distributed_engine<gl_dtypes::graph,
    graphlab::distributed_scheduler_wrapper<gl_dtypes::graph, 
    graphlab::multiqueue_fifo_scheduler<gl_dtypes::graph> > > *d_engine = new  
                graphlab::distributed_engine<gl_dtypes::graph,
                graphlab::distributed_scheduler_wrapper<gl_dtypes::graph, 
                graphlab::multiqueue_fifo_scheduler<gl_dtypes::graph> > >(dc, dg, o.ncpus);
    d_engine->set_caching(true);
    //d_engine->set_vertex_scope_pushed_updates(true);
    //d_engine->set_constant_edges(true);
    d_engine->set_default_scope(scope_range::EDGE_CONSISTENCY);
    d_engine->set_max_backlog(500);
    d_engine->set_shared_data_manager(&sdm);  
    tengine = d_engine;
  }
    //engine=tengine;
 if(tengine == NULL) {
  ...
  dc.barrier();
   ...
   // compute QQ 
  if (!tensor){ 
   sdm.set_tree_sync(RMSE, reduce_RMSE, apply_func, merge_RMSE, double(2.5),  100000000, 
                                                      scope_range::VERTEX_READ_CONSISTENCY); 
  }
  if (tensor){ 
     ...
     sdm.set_tree_sync(TIME_OFFSET, sync_QQR, apply_QQR,merge_mult, mult, 100000000, 
                       scope_range::READ_CONSISTENCY,M, M+N);
     sdm.atomic_set(TIME_OFFSET, multret);
   }
   if (BPTF){
     sdm.set_tree_sync(A_U_OFFSET, sync_MMT, apply_MMT_U,merge_QQR, QQR(), 400000000,
                       scope_range::VERTEX_READ_CONSISTENCY, 0, M);
     sdm.set_tree_sync(A_V_OFFSET, sync_MMT, apply_MMT_V,merge_QQR, QQR(), 40000000,
...
//   tengine->get_scheduler().add_tasks(um, pmf_update_function, 1);
  if (o.scheduler == "round_robin") {
    vertex_id_t zero = M+N-1;
    tengine->get_scheduler().add_task_to_all(pmf_update_function, 1.0);
    ((graphlab::distributed_scheduler_wrapper<gl_dtypes::graph, 
                graphlab::distributed_round_robin_scheduler2<gl_dtypes::graph> >*)(&(tengine->get_scheduler())))->scheduler().set_barrier_function(barrier_fn);
    ...
    tengine->get_scheduler().add_task_to_all(pmf_update_function, 1.0);
    sdm.set_constant(MAX_ITERATIONS_CONSTANT, size_t(NUM_ITERATIONS_TO_RUN));
    sdm.set_constant(IS_ROUND_ROBIN_CONSTANT, bool(false));
  }
