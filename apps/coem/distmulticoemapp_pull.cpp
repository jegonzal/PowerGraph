...
void coem_update_function(gl_types::iscope& scope, 
  ...
  if (vdata->normalizer == 0.0) {
     ...
     foreach(edge_id_t eid, edge_ids) {
      ...
      if (nb_vdata->nbcount < 1) {
          vertex_id_t nbvid = (use_outgoing?  scope.target(eid) :
                                   scope.source(eid));
          std::cout <<  "ERROR in procid " << myprocid << "  nbcount of vertex " << nbvid << " was " << nb_vdata->nbcount << std::endl;
      }
...
distmulticoemapp_pull::distmulticoemapp_pull(distributed_control * _dc, std::string npsfile, std::string contextfile, 
...
distmulticoemapp_pull::~distmulticoemapp_pull() {
  delete(g);
}
...
void distmulticoemapp_pull::start() { 
  //ncats = 100;
  //dumpcount=3;
  // retrospective_dump();
  // return;
 ...
 distributed_engine<coem_distributed_graph,
  ...
  graphlab.set_max_backlog(4);
  graphlab.set_constant_edges(true);
  //graphlab.set_vertex_scope_pushed_updates(true);
...
// Set register shared data
  ...
  // Important to enable caching only after all preparation has been done  
  graphlab.set_caching(true);
...
void distmulticoemapp_pull::output_results() {
  int cat_id = 0;
...
void distmulticoemapp_pull::load() {
 ...
 void distmulticoemapp_pull::prepare(distributed_shared_data<coem_distributed_graph>& sdm, 
                           distributed_engine<coem_distributed_graph,
...
std::map<std::string, vertex_id_t> distmulticoemapp_pull::loadVertices(short typemask, FILE * f) {
...
void distmulticoemapp_pull::loadCategories() {
...
void distmulticoemapp_pull::setseeds(std::map<std::string, vertex_id_t>& nps_map) {
  short catid = 0;
...
void distmulticoemapp_pull::loadEdges(FILE * fcont_to_nps, 
...
void distmulticoemapp_pull::loadAndConvertToBin(std::string binfile) {
  /* Open files */
...
void distmulticoemapp_pull::retrospective_dump() {
  std::string filename = appname + "_dump2.dat";
...
std::vector<double> multicoemdumper(blob_graph &g) {
  return l1_residual(g);
}


/* Used with analyzer_listener */
global_dumper distmulticoemapp_pull::dump_function() {
  return multicoemdumper;
}

std::vector<std::string> distmulticoemapp_pull::dump_headers() {
...
int distmulticoemapp_pull::dump_frequency() {
  return g->num_vertices()/5; 
}
