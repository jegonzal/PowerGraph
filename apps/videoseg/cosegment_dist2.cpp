...
template <typename Gtype, typename SDMType>
void create_graph_sdmonly(std::string archivefile,
             Gtype& g,
             SDMType & sdm,
             std::vector<uint32_t> &pix2part,
             size_t arity, size_t* featurearity,
  ...
  for (size_t cluster = 0; cluster < curval.size(); ++cluster) {
    ...
    if (curval[cluster].totalweight > 1E-5) {
      for (size_t i = 0; i < curval[cluster].mean.size(); ++i) {
        ...
        curval[cluster].variance[i] = 1E-2 + newval[cluster].variance[i] / newval[cluster].totalweight 
...
gaussian_cluster_type get_empty_gaussian_cluster(size_t arity, size_t featurearity) {
  gaussian_cluster_type vg0;
  vg0.resize(arity);
...
bool parse_command_line(int argc, char** argv, options& opts) {
  ...
  desc.add_options()
    ...
    ("arity",  boost_po::value<size_t>(&(opts.arity))->default_value(4),
     "Number of initial clusters. This must match the number of clusters in labeledimg")
...
gaussian_cluster_type 
  ...
  gaussian_cluster_type ret = get_empty_gaussian_cluster(nextlbl, features[0].size());
  std::cout << nextlbl << " colors detected";
  ret.resize(nextlbl);
  ...
  for (int i = 0;i < labels.width(); ++i) {
    for (int j = 0;j < labels.height(); ++j) {
      ...
      ret[lbl].count(1.0, features[part]);
    }
  }
  // renormalize and scale the weights
  for (size_t i  =0;i < ret.size(); ++i) {
    ret[i].normalize_to(multiplicity);
...
int main(int argc,  char *argv[]) {
  ...
  if (dc.procid() == 0) {
    ...
    std::vector<uint32_t> pix2part;
    size_t featurearity;
    gaussian_cluster_type gc ;
    if (labeledimg != "") gc= read_labelled_image(labeledbin, labeledimg, multiplicity);
    create_graph_sdmonly(inputfile, graph, shared_data, pix2part, opts.arity, &featurearity, INTRAFRAME_POTENTIAL);
    ASSERT_EQ(gc[0].mean.size(), featurearity);
    if (labeledimg != "") {
      create_shared_data(graph,shared_data, opts.arity, 
                       featurearity, 
                       INTRAFRAME_POTENTIAL, gc);
    }
    else {
      create_shared_data(graph,shared_data, opts.arity, 
                       featurearity, 
                       INTRAFRAME_POTENTIAL, get_empty_gaussian_cluster(opts.arity,featurearity));
    }
  ...
  if (dc.procid() == 0) {
    ...
    // make pictures
    makepictures(graph,pix2part, opts.arity);
  }
  dc.barrier();
}
