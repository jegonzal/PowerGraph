...
//   }
  ...
  // renumber partitions
  // frame, segmentid pair to newpartid
  std::map<std::pair<size_t, size_t>, size_t > partitions;
  size_t nextpartid = 0;
  
  std::vector<uint32_t> newpart(ret_part.size(), 0);
  for (size_t v = 0; v < ret_part.size(); ++v) {
    size_t i,j,k;
    fromvertexid(v, i,j,k);
    std::pair<size_t, size_t> idx = std::make_pair(i, ret_part[v]);
  ...
  std::vector<std::vector<size_t> > adjlist = solve_adjacencies(superpixels_idx, numparts);
  ...
  for (size_t i = 0;i < numparts; ++i) {
    features[i].resize(8*8*8, 0); 
  }
  for (size_t i = 0;i < g.num_vertices(); ++i) {
    size_t superpixid = ret_part[i];
    features[superpixid][rgb2idx_hist(g.vertex_data(i))]++;
  }
  // normalize
  for (size_t i = 0;i < numparts; ++i) {
    float sum = 0;
