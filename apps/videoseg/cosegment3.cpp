...
void makepictures(GType &g, std::vector<uint32_t> pix2part, size_t arity) {
  ...
  for (size_t i = 0;i < numimgs; ++i) {
    if (imagepath.size() > 0) {
      char cfilename[1024];
      sprintf(cfilename,"%s%d.jpg", imagepath.c_str(),(i+1));
...
int main(int argc,  char *argv[]) {
  ...
  create_shared_data(g,shared_data, opts.arity, 
                       g.vertex_data(0).features.size(), 
  ...
  graphlab::timer ti;
    ...
    for (graphlab::vertex_id_t i = 0; i < g.num_vertices(); ++i) {
      vec.push_back(i);
    }
