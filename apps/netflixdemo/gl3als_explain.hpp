...
void exp_collect_function (engine_type::context_type& context,
  ...
  vdata.top_explain2.clear();
  vdata.top_explain3.clear();
  ...
  for (size_t i = 0; i < vdata.top_pred.size(); ++i) {
    ...
    map_join<vertex_id_type, double> scores2;
    map_join<vertex_id_type, double> scores3;
    ...
    while (it != factors.train.data.end()) {
      ...
      scores2.data[id] = xi.dot(xj);
      scores3.data[id] = -(xi-xj).squaredNorm();
      ++it;
    }

    // record the 3 most important movies
    vdata.top_explain.push_back(scores.get_top_k(5));
    vdata.top_explain2.push_back(scores2.get_top_k(5));
    vdata.top_explain3.push_back(scores3.get_top_k(5));
  }
}
     ...
     if (is_user(v)) {
       ...
       for(size_t i = 0; i < top_pred.size(); ++i) {
         ...
         for (size_t j = 0; j < top_explain[i].size(); ++j) {
           ...
           sstream << v.id() << "\t" << id2movieid(gid) << "\t" << id2movieid(gid2) << "\t" << score <<"\n";
         }
       }
       return sstream.str();
     } else {
 ...
 public:
   std::string save_vertex(const graph_type::vertex_type& v) {
     ...
     if (is_user(v)) {
       ...
       const std::vector<std::vector<std::pair<double, graphlab::vertex_id_type> > >& top_explain = v.data().top_explain2;
       const std::vector<std::pair<double, graphlab::vertex_id_type> >& top_pred= v.data().top_pred;
       std::stringstream sstream;
       for(size_t i = 0; i < top_pred.size(); ++i) {
         graphlab::vertex_id_type gid = top_pred[i].second;
 ...
 public:
   std::string save_vertex(const graph_type::vertex_type& v) {
     ...
     if (is_user(v)) {
       ...
       const std::vector<std::vector<std::pair<double, graphlab::vertex_id_type> > >& top_explain = v.data().top_explain3;
       const std::vector<std::pair<double, graphlab::vertex_id_type> >& top_pred= v.data().top_pred;
       std::stringstream sstream;
       for(size_t i = 0; i < top_pred.size(); ++i) {
         graphlab::vertex_id_type gid = top_pred[i].second;
